...
// Fold common typographic variants so a smart apostrophe/quote/dash in a clip
// matches a plain one on the page (a frequent cause of a match breaking mid-list).
const FOLD = {
  '‘': "'", '’': "'", 'ʼ': "'",
  '“': '"', '”': '"',
  '–': '-', '—': '-', '−': '-',
};
function foldChar(ch) { return (FOLD[ch] || ch).toLowerCase(); }

/** Normalize for matching: fold typographic variants + collapse whitespace + lowercase. */
export function normalize(s) {
  ...
  for (let i = 0; i < str.length; i++) {
    const ch = str[i];
    if (/\s/.test(ch)) { needSpace = out.length > 0; continue; }
...
/** Split a clip into candidate segments (list items / sentences) for per-item matching. */
export function segmentsOf(text) {
  return String(text || '')
    .split(/\r?\n+|\s{2,}|(?<=[.!?])\s+/) // newlines, runs of 2+ spaces, sentence ends
    .map((s) => s.trim())
...
export function runHighlight(clipTexts) {
  try {
    ...
    if (flat.length > 3_000_000) return 0; // pathologically large page — skip (caller falls back)
    const ranges = [];
    const seen = new Set();
    const addLoc = (loc) => {
      if (!loc) return;
      const k = `${loc.startSeg}:${loc.startOff}:${loc.endSeg}:${loc.endOff}`;
    ...
    for (const text of (clipTexts || [])) {
      const whole = locate(flat, map, text);
      if (whole) { addLoc(whole); continue; }               // one contiguous highlight
      for (const seg of segmentsOf(text)) addLoc(locate(flat, map, seg)); // else per list-item/sentence
    }
