...
/**
 * Local dev instance. lana-gpt's Next.js proxy runs on :3000 by default; the
 * extension reaches the API through that proxy (never the private API host).
 * Override with chrome.storage.local `lanaDevInstance` if your dev box differs.
 */
export const DEV_INSTANCE = { url: 'http://localhost:3000', label: 'LANA GPT (dev)', kind: 'dev' };
