 ...
 *     ALSO defaults to prod (gpt.lanaai.io) so testing needs no setup; point it
 *     at a local stack only by setting chrome.storage.local `lanaDevInstance`.
 *
...
export async function defaultInstance() {
  ...
  // Dev (unpacked) builds default to PROD too, so testing hits the real cloud
  // with no setup. To point a dev build at a LOCAL stack, set chrome.storage.local
  // `lanaDevInstance` (e.g. 'http://127.0.0.1:13001'); clear it to go back to prod.
  if (isDev) {
    try {
