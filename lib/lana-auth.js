...
import { getActiveAccount, updateTokens } from './accounts.js';

/** OAuth client id the extension provisions under (matches the backend + bridge). */
const EXTENSION_CLIENT_ID = 'lana-extension';
...
export async function getEntitlements() {
  ...
  const token = (active && active.accessToken)
    ? active.accessToken
    : (await readAuth()).accessToken;
  if (!token) return {};
...
export async function getAuthState() {
  // A linked/active account (native handoff or OAuth) reflects as connected.
  const active = await getActiveAccount();
  if (active && active.accessToken) {
    const email = active.email || decodeJwtPayload(active.accessToken)?.email || null;
    return { authenticated: true, mode: 'bearer', provider: active.source || 'native', email };
  }
  const rec = await readAuth();
  let email = rec.email;
...
/** `${instanceUrl}/api/v1`, trailing-slash-safe. */
function accountApiBase(instanceUrl) {
  return `${String(instanceUrl).replace(/\/+$/, '')}/api/v1`;
}

/**
 * Single-flight per account id — concurrent authorized calls that all see an
 * expired token must NOT each POST the same single-use refresh token (that
 * replays a consumed token and can revoke the whole rotation family). Mirrors
 * the legacy path's `_refreshInFlight` discipline (review HIGH).
 */
const _activeRefreshInFlight = new Map();

/**
 * Rotate a linked account's token against ITS instance's /oauth/token
 * (grant_type=refresh_token), persisting the new pair via updateTokens. Returns
 * the new access token, or null if refresh is dead (user must re-link). Never
 * throws — a transient failure just returns null so the caller keeps the current
 * token / surfaces the eventual 401. Coalesced per account id.
 */
function refreshActiveAccount(active) {
  if (!active || !active.id) return Promise.resolve(null);
  const inflight = _activeRefreshInFlight.get(active.id);
  if (inflight) return inflight;
  const p = (async () => {
    // Re-read from the store inside the critical section: a just-completed
    // refresh (same-call proactive, or a racing call) may already have rotated
    // the token, so use the CURRENT refresh token, never the captured stale one.
    const cur = await getActiveAccount();
    const rt = (cur && cur.id === active.id) ? cur.refreshToken : active.refreshToken;
    if (!rt) return null;
    try {
      const resp = await fetch(`${accountApiBase(active.instanceUrl)}/oauth/token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'refresh_token', refresh_token: rt, client_id: EXTENSION_CLIENT_ID }),
      });
      if (!resp.ok) return null; // invalid_grant / rotation failure → dead refresh
      const data = await resp.json().catch(() => null);
      if (!data || !data.access_token) return null;
      // Default a sane TTL if the server omits expires_in, so the proactive
      // near-expiry check doesn't fire on every subsequent call (review LOW).
      const expiresIn = (typeof data.expires_in === 'number' && data.expires_in > 0) ? data.expires_in : 3600;
      await updateTokens(active.id, { accessToken: data.access_token, refreshToken: data.refresh_token, expiresIn });
      return data.access_token;
    } catch {
      return null;
    }
  })().finally(() => _activeRefreshInFlight.delete(active.id));
  _activeRefreshInFlight.set(active.id, p);
  return p;
}

/**
 * authorizedFetch for a linked/active account: targets the account's own
 * instance + bearer, proactively refreshes near expiry, and retries ONCE on 401
 * after a refresh. No cross-instance risk — the account's instance IS the target.
 */
async function activeAccountFetch(active, path, init = {}) {
  const url = `${accountApiBase(active.instanceUrl)}/${String(path).replace(/^\/+/, '')}`;
  let token = active.accessToken;
  if (active.expiresAt && Date.now() >= active.expiresAt - REFRESH_SKEW_MS) {
    const fresh = await refreshActiveAccount(active);
...
export async function authorizedFetch(path, init = {}) {
  // Multi-account: if the user has a linked/active account (native handoff or
  // OAuth), use ITS instance + token. Falls back to the legacy single-account
  // record when no account is linked. See lib/accounts.js.
  const active = await getActiveAccount();
  if (active && active.accessToken) return activeAccountFetch(active, path, init);

  const rec = await readAuth();
  if (!rec.mode) {
    // Expected state (user hasn't authorized yet) — mark it so the SW doesn't log
    // it as a red error in the extension's error panel.
    const e = new Error('Not signed in to LANA. Connect your account in Settings.');
