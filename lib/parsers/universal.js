    ...
    if (path.endsWith('conversations.json') || path.match(/conversations\.json$/i)) {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        if (data[0].chat_messages) {
          // Also extract projects.json if present
          let projects = null;
          for (const { path: pPath, entry: pEntry } of jsonFiles) {
            if (pPath.endsWith('projects.json') || pPath.match(/projects\.json$/i)) {
              try {
                const pText = await pEntry.async('string');
                projects = JSON.parse(pText);
              } catch { /* ignore */ }
              break;
            }
          }
          return { source: 'claude', conversations: parseClaude(data), projects };
        }
      }
    }
  }
