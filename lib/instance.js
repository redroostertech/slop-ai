...
import { defaultInstance as envDefaultInstance, PROD_INSTANCE } from './env.js';

const INSTANCE_KEY = 'lanaInstance';

/** Built-in presets (on-prem is a free-form URL entry, dev-/admin-only UI). */
export const INSTANCE_PRESETS = [
  { url: 'https://gpt.lanaai.io', label: 'LANA GPT', kind: 'cloud' },
];

/**
 * The default instance when the user hasn't chosen one.
 *
 * NOTE: this is the *production* constant, kept for callers that need a
 * synchronous value (e.g. classifying `kind` in {@link setInstance}). The
 * actual runtime default is environment-derived — prod builds get this, dev
 * builds get a local instance — so prefer {@link getInstance}, which resolves
 * the env, over reading this directly.
 */
export const DEFAULT_INSTANCE = PROD_INSTANCE;
...
export function normalizeInstanceUrl(raw) {
  ...
  // http is cleartext, so tokens/credentials would be sent in the clear. Only
  // permit it for loopback (local dev / a box on the same machine), never a
  // routable host — a plaintext public/LAN instance is a MITM credential leak.
  if (u.protocol === 'http:' && !isLoopbackHost(u.hostname)) {
    throw new Error('Instance must use https (http is allowed only for localhost).');
  }
  // Keep origin + any path prefix (some on-prem deployments live under a path),
  // drop a trailing slash, and strip a pasted `/api/v1` so `${base}/api/v1/...`
  // never doubles to `/api/v1/api/v1/...`.
  let path = u.pathname.replace(/\/+$/, '').replace(/\/api\/v1$/i, '');
  return (u.origin + path).replace(/\/+$/, '');
}

/** Is this hostname loopback (127.0.0.0/8, ::1, localhost)? */
...
export async function getInstance() {
  ...
  // Nothing chosen → derive from the build environment (prod cloud vs local dev)
  // so the end user never has to supply a URL.
  return await envDefaultInstance();
}
