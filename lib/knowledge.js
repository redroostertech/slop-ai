...
export async function getAllTopicsWithCounts() {
  ...
  // Count project-tagged conversations per topic (for unsummarized project convs)
  const projectTopicIds = new Set(topics.filter(t =>
    t.metadata?.source === 'chatgpt-project' || t.metadata?.source === 'claude-project'
  ).map(t => t.id));
  let projectConvCounts = {};
  if (projectTopicIds.size > 0) {
    const conversations = await dbGetAll('conversations');
    for (const c of conversations) {
      const ptid = c.metadata?.projectTopicId;
      if (ptid && projectTopicIds.has(ptid)) {
        projectConvCounts[ptid] = (projectConvCounts[ptid] || 0) + 1;
      }
    }
  }

  return topics.map(t => {
    const summaryCount = t.summaryIds?.length || 0;
    const projectConvCount = projectConvCounts[t.id] || 0;
    return {
      ...t,
      summaryCount: Math.max(summaryCount, projectConvCount)
    };
