 ...
 * assumptions.
 *
 * Tokens are provisioned either by the desktop app over the loopback PAC bridge
...
/** Is a host loopback (safe for cleartext http in dev)? */
function isLoopbackHost(h) {
  const host = (h || '').replace(/^\[|\]$/g, '');
  return host === 'localhost' || host === '::1' || /^127\.\d{1,3}\.\d{1,3}\.\d{1,3}$/.test(host);
}

/** The secure origin for an instance URL: https anywhere, or http for loopback
 *  ONLY (dev). THROWS otherwise — a bearer token must never be storable against
 *  (and later sent to) a cleartext non-loopback / attacker origin (review HIGH).
 *  Mirrors lib/instance.js's scheme policy. */
function secureOrigin(instanceUrl) {
  const u = new URL(instanceUrl); // throws if unparseable
  if (u.protocol === 'https:') return u.origin;
  if (u.protocol === 'http:' && isLoopbackHost(u.hostname)) return u.origin;
  throw new Error('instanceUrl must be https (http allowed only for loopback)');
}
...
function assertValidBundle(b) {
  ...
  secureOrigin(b.instanceUrl); // throws on non-https/unparseable
...
export async function linkAccount(bundle) {
  ...
  const origin = secureOrigin(bundle.instanceUrl);
...
export async function updateTokens(id, { accessToken, refreshToken, expiresIn }) {
  ...
  // Only overwrite the refresh token with a real new one — a null/absent value
  // (server didn't rotate) must NOT wipe the still-valid refresh token (review LOW).
  if (typeof refreshToken === 'string' && refreshToken) a.refreshToken = refreshToken;
