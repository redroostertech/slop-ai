...
export function detectFields(doc) {
  // input + textarea only. <select> can't take an arbitrary typed value and
  // crashes the input/textarea value-setter path in applyField, so it's excluded.
...
export function applyField(doc, approved) {
  ...
  // Only input/textarea are supported. Calling the input value-setter on any
  // other element (e.g. a <select>) throws "Illegal invocation".
