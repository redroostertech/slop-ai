 ...
 *
 * Single-flight: concurrent callers share one engine creation (mirrors the
 * offscreen `_enginePromise` guard) — otherwise two overlapping generateLocal
 * calls each build an engine, double-downloading the model and risking WebGPU OOM.
 */
async function getWebLlmEngine() {
  if (_webllmEngine) return _webllmEngine;
  if (_webllmEnginePromise) return _webllmEnginePromise;
  _webllmEnginePromise = (async () => {
    ...
    if (c && c.model && c.model_id && c.model_lib) {
      return webllm.CreateMLCEngine(c.model_id, {
        appConfig: {
          model_list: [
    ...
    return webllm.CreateMLCEngine(localModelId || 'Qwen2.5-1.5B-Instruct-q4f16_1-MLC');
  })();
  try {
    _webllmEngine = await _webllmEnginePromise;
    return _webllmEngine;
  } catch (err) {
    _webllmEnginePromise = null; // allow retry
    throw err;
  }
}
