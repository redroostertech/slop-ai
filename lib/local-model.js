 ...
 *     Used in a DOCUMENT with WebGPU (e.g. the sidepanel). REQUIRES the WebLLM
 *     runtime vendored (CSP blocks CDN imports) and exposed as `self.webllm`.
 *
...
const BACKENDS = {
  ...
  'offscreen-webllm': {
    ...
    async generate(messages, options) {
      ...
      // only the serializable knobs cross into the offscreen document.
...
async function getWebLlmEngine() {
  ...
  if (c && c.model && c.model_id && c.model_lib) {
    _webllmEngine = await self.webllm.CreateMLCEngine(c.model_id, {
      appConfig: {
        model_list: [
    ...
    _webllmEngine = await self.webllm.CreateMLCEngine(
