...
/**
 * Grab ONLY the current text selection (plus page url/title) from the page.
 * Lighter than injectedExtractReadable — used by the "Clip selection" affordance
 * where the user has explicitly highlighted the text they want. Runs in the page.
 * @returns {{title: string, url: string, selection: string}}
 */
function injectedGetSelection() {
  const doc = document;
  let selection = '';
  try {
    selection = (window.getSelection && window.getSelection().toString()) || '';
  ...
  const title =
    ...
    doc.title ||
    '';
  return { title: String(title).trim(), url, selection: selection.trim() };
}
...
/**
 * Inject the selection-grab fn into the active tab and return the current text
 * selection (with page url/title). Null if no scriptable tab / no result.
 * @returns {Promise<{title: string, url: string, selection: string}|null>}
 */
export async function selectionActiveTab(tabId) {
  const tab = await resolveTab(tabId);
  const injection = await chrome.scripting.executeScript({
    target: { tabId: tab.id },
    func: injectedGetSelection,
  });
  const result = injection && injection[0] ? injection[0].result : null;
  return result || null;
}
