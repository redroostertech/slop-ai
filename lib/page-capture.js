 ...
 *
 * Requires the `scripting` + `activeTab` permissions (already in manifest) and,
 * for non-AI hosts, host access granted on demand (activeTab covers the
 * user-gesture case; broad capture needs optional host permissions — see the
 * extension's permission flow).
 *
 * @module lib/page-capture
 */

/**
 * Resolve the active tab, or throw if there isn't a scriptable one.
 ...
 */
export async function detectFieldsActiveTab() {
  const tab = await getActiveTab();
  const injection = await chrome.scripting.executeScript({
    target: { tabId: tab.id },
 ...
 */
export async function applyFieldActiveTab(approved) {
 ...
 */
export async function activeTabOrigin() {
  const tab = await getActiveTab();
  try {
