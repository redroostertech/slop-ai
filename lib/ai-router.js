/**
 * @fileoverview Multi-Provider AI Router (bring-your-own-key chain).
 *
 * Routes AI inference through a priority-ordered chain of providers:
 * LANA AI -> OpenAI -> Claude -> Gemini. Each provider has its own adapter that
 * normalizes request/response formats. Providers are stored in
 * chrome.storage.local under the key `aiProviders`.
 *
 * NOTE: the PRIMARY inference path for the extension is the hybrid cascade in
 * lib/router.js (on-device model → LANA JWT passthrough). This module is the
 * bring-your-own-key fallback chain and the connectivity test surface. The
 * `lana` adapter here uses the same JWT passthrough (via lib/lana-client), so a
 * user does NOT paste a Forge/LANA key.
 *
...
/**
 * LANA AI adapter — delegates to the JWT passthrough (lib/lana-client), so the
 * extension authenticates as the signed-in user and the instance forwards to
 * Forge server-side. No API key on the provider object.
 */
async function lanaAdapter(provider, messages, options) {
  // The passthrough only accepts tier ∈ {auto,fast,reasoning}; anything else
  // (e.g. the legacy 'lana-default' defaultModel) maps to 'auto'.
  const requested = options.model || provider.defaultModel;
  const tier = ['auto', 'fast', 'reasoning'].includes(requested) ? requested : 'auto';
  const result = await inferenceComplete(messages, {
